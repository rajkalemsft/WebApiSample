...
#> 
    ...
    $webAPIName = "RKLab-TodoList-webapi-daemon-v2"
    $clientAppName = "RKLab-daemon-console-v2"
   # Create the service AAD application
   Write-Host "Creating the AAD application ($webAPIName)"
   $serviceAadApplication = New-AzureADApplication -DisplayName $webAPIName `
   ...
   Write-Host "Done creating the service application ($webAPIName)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>$webAPIName</a></td></tr>" -Path createdApps.html

   # Create the client AAD
   Write-Host "Creating the AAD application $clientAppName"
   # Get a 2 years application key for the client Application
   $pw = ComputePassword
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInYears 2 -pw $pw
   $clientAppKey = $pw
   $clientAadApplication = New-AzureADApplication -DisplayName $clientAppName `
                                                  -ReplyUrls "https://daemon" `
                                                  -IdentifierUris "https://$tenantName/$clientAppName" `
   ...
   Write-Host "Done creating the client application ($clientAppName)"

   # URL of the AAD application in the Azure portal
   # Future? $clientPortalUrl = "https://portal.azure.com/#@"+$tenantName+"/blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/Overview/appId/"+$clientAadApplication.AppId+"/objectId/"+$clientAadApplication.ObjectId+"/isMSAApp/"
   $clientPortalUrl = "https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/CallAnAPI/appId/"+$clientAadApplication.AppId+"/objectId/"+$clientAadApplication.ObjectId+"/isMSAApp/"
   Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>$clientAppName</a></td></tr>" -Path createdApps.html

   $requiredResourcesAccess = New-Object System.Collections.Generic.List[Microsoft.Open.AzureAD.Model.RequiredResourceAccess]

   # Add Required Resources Access (from 'client' to 'service')
   Write-Host "Getting access from 'client' to 'service'"
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName $webAPIName `
